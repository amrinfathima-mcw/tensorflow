...
void* Init(TfLiteContext* context, const char* options, size_t options_len) {
  ...
  const TfLiteStablehloCSortParams* params =
      reinterpret_cast<const TfLiteStablehloSortParams*>(options);
  data->subgraph_index = params->comparator_subgraph_index;
  return data.release();
}
...
TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  for (int i = 0; i < num_inputs; ++i) {
    ...
    TfLiteTensor* subgraph_input = comparator_subgraph->tensor(input_idx);
  ...
  op_state->subgraph_has_dynamic_output_tensors |=
      comparator_subgraph->HasDynamicTensors();
  ...
  for (int i = 0; i < num_outputs; ++i) {
    ...
    if (op_state->subgraph_has_dynamic_output_tensors) {
      ...
      TfLiteTensor* subgraph_output =
          comparator_subgraph->tensor(comparator_subgraph->outputs()[i]);
...
TfLiteStatus Eval_dynamic(TfLiteContext* context, TfLiteNode* node,
  ...
  TF_LITE_ENSURE_OK(
      context, DeepOrShallowCopyTensorsShapeTypeData(
                   ...
                   comparator_subgraph, comparator_subgraph->inputs()));

  // Invoke decomposition_subgraph subgraph
  TF_LITE_ENSURE_OK(context, comparator_subgraph->Invoke());
  for (int tensor_index : comparator_subgraph->outputs()) {
    comparator_subgraph->EnsureTensorDataIsReadable(tensor_index);
  }

  // subgraph->outputs -> node->outputs
  TF_LITE_ENSURE_OK(context,
                    DeepCopyTensorsShapeTypeData(
                        context, node, comparator_subgraph,
                        comparator_subgraph->outputs(), this_subgraph,
                        TfLiteIntArrayView(node->outputs), true));

  for (int i = 0; i < num_outputs; ++i) {
    const int input_pos = OutputIsInput(comparator_subgraph->outputs()[i],
                                        comparator_subgraph->inputs());
    if (input_pos != -1) {
...
TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) {
  ...
  if (!this_subgraph->ShouldPreserveAllTensors()) {
    TF_LITE_ENSURE_OK(context, comparator_subgraph->ReleaseMemory());
  }
