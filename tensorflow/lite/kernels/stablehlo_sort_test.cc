 ...
 public:
  SortOpModel(const std::vector<TensorData>& inputs,
    ...
    for (int i = 0; i < num_inputs; ++i) {
      ...
      outputs_.push_back(
          AddOutput(TensorData(inputs[0].type, GetShape(inputs_[0]),0,0,inputs[0].scale,inputs[0].zero_point)));
    }
...
TEST(StablehloSortOpTest, SortWorks1) {
  ...
  SortOpModel model(inputs, params, comparison_direction, num_inputs, lhs, rhs, kTfLiteInt32);
  model.SetInput<int>(
...
TEST(StablehloSortOpTest, SortWorks2) {
  ...
  SortOpModel model(inputs, params, comparison_direction, num_inputs, lhs, rhs, kTfLiteFloat32);
...
TEST(StablehloSortOpTest, QuantizedSortWorks1) {
  ...
  int rhs = 1;
  std::vector<TensorData> inputs(num_inputs,
                                 {TensorType_INT8, {2, 3}, -227.0f, 228.0f, 1.5f, 1});
