...
TfLiteStatus SortOp(TfLiteContext* context, TfLiteNode* node,
                    ...
                    int32_t dimension, bool is_stable,
                    std::vector<TfLiteTensor*>& results) {
  using StorageT = DataType;
  ...
  std::vector<DataType> args(operands.size());
...
TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) {
  ...
  if (data_type == kTfLiteFloat32) {
    ...
    return SortOp<int8_t>(context, node, inputs, dimension, is_stable, outputs);
  } else if (data_type == kTfLiteInt16) {
    return SortOp<int16_t>(context, node, inputs, dimension, is_stable,
                           outputs);
